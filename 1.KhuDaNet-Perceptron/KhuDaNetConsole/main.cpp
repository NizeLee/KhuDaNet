...
int main(int argc, char* argv[])
{
	...
	gNetwork.AddLayer(CKhuDaNetLayerOption(KDN_LT_FC | KDN_LT_OUTPUT, 0, 1, 0, 0, 0, KDN_AF_SIGMOID, 0.15));
	...
	while(1)
	{
		...
		for(i = 0 ; i < nBatch ; ++i)
		{
			...
			{
				OutputList[i][j] = 0;
				if(gMnistTrainOutput[nIndex+i] > 4) OutputList[i][j] = 1;
			}
		}
		...
		if(nIndex+nBatch == gMnistTrainCnt)
		{
			...
			for(i = 0 ; i < gMnistTestCnt ; i++)
			{
				...
				if((gMnistTestOutput[i]>4?1:0) == nResult) nTP++;
			}
