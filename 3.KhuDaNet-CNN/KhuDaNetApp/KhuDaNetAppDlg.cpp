...
void CKhuDaNetAppDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_NETWORK_TYPE, m_nNetworkType);
...
void CKhuDaNetAppDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if(nIDEvent == TIMER_ID_MNIST_TRAIN)
	{
		...
		if((m_Network.m_Layers[0]->m_LayerOption.nLayerType & KDN_LT_CON) && (m_Network.m_Layers[2]->m_LayerOption.nLayerType & KDN_LT_POOL))
		{
			CClientDC dc(AfxGetMainWnd());

			CKhuDaNetLayer *InputLayer = m_Network.m_Layers[0];
			double Max = 0;
			for(int y = 0 ; y < InputLayer->m_LayerOption.nH ; ++y)
				for(int x = 0 ; x < InputLayer->m_LayerOption.nW ; ++x)
					if(InputLayer->m_NodeCnnImage[0][y][x] > Max) 
						Max = InputLayer->m_NodeCnnImage[0][y][x];

			for(int y = 0 ; y < InputLayer->m_LayerOption.nH ; ++y)
				for(int x = 0 ; x < InputLayer->m_LayerOption.nW ; ++x)
				{
					COLORREF c = RGB((int)(InputLayer->m_NodeCnnImage[0][y][x]/Max*255), (int)(InputLayer->m_NodeCnnImage[0][y][x]/Max*255), (int)(InputLayer->m_NodeCnnImage[0][y][x]/Max*255));

					dc.SetPixelV(CPoint(x+15, y+15), c);
				}

			CKhuDaNetLayer *ThirdLayer = m_Network.m_Layers[2];

			for(int j = 0 ; j < ThirdLayer->m_LayerOption.nImageCnt ; ++j)
			{
				double Max = 0;
				for(int y = 0 ; y < ThirdLayer->m_LayerOption.nH ; ++y)
					for(int x = 0 ; x < ThirdLayer->m_LayerOption.nW ; ++x)
						if(ThirdLayer->m_NodeCnnImage[j][y][x] > Max) 
							Max = ThirdLayer->m_NodeCnnImage[j][y][x];
				...
				for(int y = 0 ; y < ThirdLayer->m_LayerOption.nH ; ++y)
					...
					{
						int dx, dy;
						COLORREF c = RGB((int)(ThirdLayer->m_NodeCnnImage[j][y][x]/Max*255), (int)(ThirdLayer->m_NodeCnnImage[j][y][x]/Max*255), (int)(ThirdLayer->m_NodeCnnImage[j][y][x]/Max*255));

						for(dy = 0 ; dy < 4 ; ++dy)
							for(dx = 0 ; dx < 4 ; ++dx)
							{
								dc.SetPixelV(CPoint(x*4+dx+15 + (ThirdLayer->m_LayerOption.nW*4+20)*j, y*4+dy+32+20+15), c);
							}
					}
			}
		}
...
void CKhuDaNetAppDlg::OnBnClickedRandomTest()
{
	...
	if(m_Network.IsNetwork())
	{
		...
		if(m_MnistTestOutput[nSel] == nResult) 
			sprintf(strResult, "Result: %d (true)", nResult);
		else
			sprintf(strResult, "Result: %d (false)", nResult);
